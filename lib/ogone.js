...
function OgoneError(response) {
    ...
    this.message = "(NCError: " + (response.ncerror || "Unknown") + ') ' + (response.ncerrorplus || '');
...
function Request(url, json, schema) {
    ...
    if (!result.valid) {
        var e = result.errors.map(function (err) {
            return ['Property', err.property, err.message].join(' ');
        }).join('; ');
        throw new Error(e);
    }
...
Request.prototype._prepare = function(query) {
    if (this.algorithm && this.key) {
        query.shasign = Request.hashify(this.algorithm, this.key, query);
    }
...
Request.prototype._send = function(callback) {
    ...
    request.post({
        ...
        this.parser.parseString(body, function (err, result) {
            if (err) return callback(err);
            // if (!result['@']) return callback(new Error("Unknown response:"), null, result);
            if (!result['@'] && !result.ncresponse && !result.ncresponse['$']) {
                return callback(new Error("Unknown response:"), null, result);
            }
            // var normalize = Request.lower(result['@']);
            var normalize = Request.lower(result['@'] || result.ncresponse['$']);
...
Request.hashify = function(algorithm, key, obj) {
